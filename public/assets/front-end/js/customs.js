...
function addToCart(form_id = "add-to-cart-form", redirect_to_checkout = false) {
    ...
    const productId =
        $(".action-add-to-cart-form").data("product-id") ||
        $("#" + form_id)
            .find('input[name="id"]')
            .val() ||
        $("#" + form_id)
            ...
            try {
                ...
                if (response.status == 1) {
                    ...
                    $.get(window.location.href, function (res) {
                        ...
                        const newSidebar = tempDOM.querySelector(
                            "#cart-sidebar .cart-sidebar-content"
                        );
                        if (newSidebar) {
                            document.querySelector(
                                "#cart-sidebar .cart-sidebar-content"
                            ).innerHTML = newSidebar.innerHTML;
                            cartListQuantityUpdateInit();
                        }
                    ...
                    $('.in_cart_key[name="key"]').val(response.in_cart_key);
                    $(".action-add-to-cart-form").html(
                        $(".action-add-to-cart-form").data("update-text")
                    );
                    $(".call-when-done").click();
            ...
            $.post(
                ...
                function (response) {
                    if (response["status"] == 0) {
                        ...
                        if (
                            response.cart &&
                            response.cart.total_quantity !== undefined
                        ) {
                            $("#cart-count").text(response.cart.total_quantity);
                        }

                        // ✅ Sidebar replacement
                        $("#cart-sidebar .cart-sidebar-content").replaceWith(
                            response.html
                        );
                        cartListQuantityUpdateInit();
...
$(document).ready(function () {
    ...
    function loadSelectedProducts() {
        try {
            ...
            console.error(
                "Error parsing saved products from local storage:",
                error
            );
            return [];
        }
    ...
    function handleCompareClick(productId, productCategoryId) {
        ...
        const currentCategory =
            selectedProducts.length > 0 ? selectedProducts[0].categoryId : null;
...
$(document).ready(function () {
    ...
    function renderProducts() {
        ...
        $(".compare-product_item").each(function () {
            ...
            if (
                !storedProducts.some(function (product) {
