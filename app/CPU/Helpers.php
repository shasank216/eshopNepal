...
class Helpers
{
    ...
    {
        $x = ['kg', 'pc', 'gms', 'ltrs', 'ml', 'ctn', 'dozen', 'pkt', 'nos', 'sq mt', 'tonne', 'meter', 'centimeter'];
        return $x;
    }
    ...
    public static function get_price_range_with_discount($product)
    {
        ...
        if ($discounted_lowest_price == $discounted_highest_price) {
            if ($discounted_lowest_price == self::currency_converter($lowest_price)) {
                ...
                return $discounted_lowest_price . " <del>" . self::currency_converter($lowest_price) . "</del> ";
            }
        }
        return $discounted_lowest_price . " <del>" . self::currency_converter($lowest_price) . "</del> " . ' - ' . $discounted_highest_price . " <del>" . self::currency_converter($highest_price) . "</del> ";
    }
    ...
    public static function send_push_notif_to_topic($data, $topic = 'sixvalley')
    {
        ...
        $users = User::whereNotNull('cm_firebase_token')->get();
        ...
        foreach ($users as $user) {
            ...
            // You might want to log the results or handle errors here
        }

        // You can return a summary of the notifications sent if needed
        return "Bulk push notifications sent to all users with FCM tokens.";
    }
...
if (!function_exists('get_seller')) {
    function get_seller($request = null)
    {
        ...
        if (session()->has('seller_id') && !session('is_guest')) {
            return User::find(session('seller_id'));
        }
