...
class HomeController extends Controller
{
    ...
    public function default_theme(Request $request)
    {
        ...
        //     });
        // old top seller code ends

        // new top seller code
        $userLat = $request->query('lat');
        $userLng = $request->query('lng');

        // Step 1: Fetch all top sellers (you may fetch more initially for distance filtering)
        $top_sellers = $this->seller->approved()
            ...
            ->take(30) // Fetch more for better filtering
            ->get();

        // Step 2: Add ratings, reviews
        $top_sellers->map(function ($seller) {
            $seller->product?->map(function ($product) {
                $product['rating'] = $product->reviews->pluck('rating')->sum();
                $product['review_count'] = $product->reviews->count();
        ...
        // Step 3: If user location is present, geocode seller addresses and calculate distances
        if ($userLat && $userLng) {
            $filtered = [];
            ...
            // If no location, fallback to top 12 by order count
            $top_sellers = $top_sellers->take(12);
        }

        if ($request->ajax()) {
            return view('partials._nearest_vendors', compact('top_sellers'))->render();
        }
        // new top seller code ends
