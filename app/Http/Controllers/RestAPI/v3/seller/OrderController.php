...
class OrderController extends Controller
{
    ...
    public function assign_delivery_man(Request $request)
    {
        ...
        \Log::info('status change customer fcm token: ' . $userFcmToken);

        if($userFcmToken){
            $data = [
                'title' => 'New Assigned Order',
                'description' => 'Your order #' . $order->id . ' has been to ' . $deliveryMan->f_name . ' ' . $deliveryMan->l_name,
            ];

            try{
                $firebaseService->sendToDevice($userFcmToken, $data['title'], $data['description']);

            }catch(\Exception $e){

                \Log::info('delivery status push error: ' . $e->getMessage());
            }
        ...
        //     $data = [
        //         'title' => 'New Order',
        //         'description' => 'You have changed the status of order #' . $order->id . ' to ' . $request['status'],
        //     ];

        //     $firebaseService->sendToDevice($sellerFcmToken, $data['title'], $data['description']);
        ...
        if($deliveryMan){
            $data = [
                'title' => 'New Assigned Order',
                'description' => 'You have been assigned to order #' . $order->id,
            ];

            $firebaseService->sendToDevice($deliveryMan->fcm_token, $data['title'], $data['description']);
        }
    ...
    public function order_detail_status(Request $request)
    {
        ...
        if($userFcmToken){
            $data = [
                'title' => 'Order Status',
                'description' => 'The status of order #' . $order->id . ' to ' . $request['status'],
            ];
