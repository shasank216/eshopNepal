...
class SocialAuthController extends Controller
{
    ...
    //         $emailFromProvider = $data['email'] ?? $email;
    //         if (isset($user) == false) {
    ...
    public function social_login(Request $request)
{
    ...
    try {
        if ($provider == 'google') {
            ...
            $res = $client->request('GET', 'https://graph.facebook.com/' . $unique_id . '?access_token=' . $token . '&fields=id,name,email');
            $data = json_decode($res->getBody()->getContents(), true);

        } elseif ($provider == 'apple') {
            // Get Apple credentials from settings
            $apple_login = BusinessSetting::where(['type' => 'apple_login'])->first();
            if ($apple_login) {
                $apple_login = json_decode($apple_login->value)[0];
            }
            ...
            $apple_client_secret = \Firebase\JWT\JWT::encode([
                'iss' => $teamId,
                'iat' => $iat,
                'exp' => $exp,
                'aud' => $aud,
            ...
            $redirect_uri = $apple_login->redirect_url ?? 'https://example.com/apple-callback';

            // Exchange authorization code for tokens
            $res = \Illuminate\Support\Facades\Http::asForm()->post('https://appleid.apple.com/auth/token', [
                'grant_type' => 'authorization_code',
                'code' => $unique_id,
                'redirect_uri' => $redirect_uri,
                'client_id' => $sub,
    ...
    $emailFromProvider = $data['email'] ?? null;
    if (!$emailFromProvider) {
        // Apple may not always return email → fallback to generated one
        $emailFromProvider = $unique_id . '@apple.com';
    }

    // Split name
    $fullName = $data['name'] ?? $emailFromProvider;
    $name = explode(' ', $fullName);
    $first_name = count($name) > 1 ? implode(" ", array_slice($name, 0, -1)) : $name[0];
    $last_name  = count($name) > 1 ? end($name) : '';

    // Check if user exists
    $user = User::where('email', $emailFromProvider)->first();
    ...
    if (!$user) {
        ...
        $user = User::create([
            ...
            'social_id' => $data['id'] ?? $data['sub'] ?? $unique_id,
            'is_phone_verified' => 0,
            'is_email_verified' => 1,
            'referral_code' => Helpers::generate_referer_code(),
            'temporary_token' => \Illuminate\Support\Str::random(40),
        ]);
    } else {
        // Update existing
        $user->temporary_token = \Illuminate\Support\Str::random(40);
        $user->login_medium = $provider;
        $user->social_id = $user->social_id ?? ($data['id'] ?? $data['sub'] ?? $unique_id);
        $user->save();
    }

    // Generate Passport access token (direct login)
    $tokenResult = $user->createToken('customer_auth')->accessToken;
