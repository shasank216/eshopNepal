    ...
    <tbody>
        @foreach ($data['orders'] as $key => $order)
            ...
            <tr>
                ...
                <td> {{ ucwords($order->is_guest == 0 ? ($order?->customer?->f_name ?? translate('not_found')) . ' ' . $order?->customer?->l_name : translate('guest_customer')) }}
                </td>
                <td> {{ ucwords($order?->seller_is == 'seller' ? $order?->seller?->shop->name ?? translate('not_found') : translate('inhouse')) }}
                </td>
                <td> {{ $order->total_qty }} </td>
                <td> {{ setCurrencySymbol(amount: usdToDefaultCurrency(amount: $order?->total_price ?? 0), currencyCode: getCurrencyCode()) }}
                </td>
                <td> {{ setCurrencySymbol(amount: usdToDefaultCurrency(amount: $order?->total_discount ?? 0), currencyCode: getCurrencyCode()) }}
                </td>
                <td> {{ setCurrencySymbol(amount: usdToDefaultCurrency(amount: $order?->discount_amount ?? 0), currencyCode: getCurrencyCode()) }}
                </td>
                <td> {{ setCurrencySymbol(amount: usdToDefaultCurrency(amount: $extra_discount), currencyCode: getCurrencyCode()) }}
                </td>
                <td> {{ setCurrencySymbol(amount: usdToDefaultCurrency(amount: ($order?->total_price ?? 0) - ($order?->total_discount ?? 0) - ($order?->discount_amount ?? 0) - ($order->is_shipping_free == 0 ? $extra_discount : 0)), currencyCode: getCurrencyCode()) }}
                </td>
                <td> {{ setCurrencySymbol(amount: usdToDefaultCurrency(amount: $order?->total_tax ?? 0), currencyCode: getCurrencyCode()) }}
                </td>
                <td> {{ setCurrencySymbol(amount: usdToDefaultCurrency(amount: $order->is_shipping_free == 0 ? $order?->shipping_cost ?? 0 : 0), currencyCode: getCurrencyCode()) }}
                </td>
                <td> {{ setCurrencySymbol(amount: usdToDefaultCurrency(amount: $totalAmount ?? 0), currencyCode: getCurrencyCode()) }}
                </td>
